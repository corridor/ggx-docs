"""
Based on mkdocs-with-pdf, create a theme handler that can add the appropriate CSS and
Add the PDF to our documentation correctly.
Ref: https://github.com/orzih/mkdocs-with-pdf/blob/v0.8.0/mkdocs_with_pdf/themes/generic.py
"""

from bs4 import BeautifulSoup
from mkdocs_with_pdf.themes import material


def get_stylesheet(debug_html: bool) -> str:  # noqa: FBT001 -- Needs to be as per mkdoc-with-pdf argspec
    return material.get_stylesheet(debug_html)


def get_script_sources() -> list:
    return material.get_script_sources()
...
def inject_link(html: str, href: str) -> str:
    ...
    if nav:
        a = soup.new_tag("a", href=href, title="Download as PDF", **{"class": "md-header-nav__button md-icon"})
        a.append(
            BeautifulSoup(
                """
