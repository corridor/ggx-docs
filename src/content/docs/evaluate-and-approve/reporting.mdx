...
- **`job`** — a handle to the run and its context: `job.parameters` (the values from the Parameter Schema), `job.current` (the object under test), and `job.current.name` (its name). For **comparison** jobs, `job.challengers` exposes the challenger objects.
...
:::note[ID column for comparisons]
Keep an **ID column** in your data so challenger results can be merged back onto the current object's rows. Comparison and validation jobs render challenger and benchmark names dynamically, so read them from `job.challengers` rather than hard-coding.
:::
...
A metric-style output simply returns a value keyed by role, e.g. `{"current": accuracy_value}`, so it lines up next to challengers in comparison jobs.
